...
test('EventBridge Rule forwards to FIFO SQS with correct JSON transformation', () => {
  template.hasResourceProperties('AWS::Events::Rule', {
    ...
    Targets: Match.arrayWith([
      Match.objectLike({
        SqsParameters: {
          MessageGroupId: "$.detail.fifoMetadata.messageGroupId", // 👈 check MessageGroupId for FIFO
        },
        InputTransformer: {
          InputPathsMap: {
            'detail-payload-decisionOutComeRequestId': '$.detail.payload.decisionOutComeRequestId',
            'detail-payload-eventDataList-0--decisionOutComeId': '$.detail.payload.eventDataList[0].decisionOutComeId',
          },
          InputTemplate: Match.stringLikeRegexp(
            '.*<detail-payload-decisionOutComeRequestId>.*<detail-payload-eventDataList-0--decisionOutComeId>.*'
          ),
        },
      }),
    ]),
